...
import { listAgents, createAgent, updateAgent, deleteAgent, me, User, canWrite, isAdmin, getAccessToken } from "../lib/api";
...
export default function AgentsPage() {
  ...
  async function onActivate(agent: Agent) {
    try {
      ...
      const updated = await updateAgent(agent.agent_id, { status: agent.status === 'active' ? 'inactive' : 'active' });
      setItems(prev => prev.map(a => a.agent_id === agent.agent_id ? updated : a));
    ...
    <div>
      ...
      {canWrite(currentUser) ? (
        <form onSubmit={onCreate} style={{ display: 'grid', gap: 8, maxWidth: 520, marginBottom: 24 }}>
          ...
          <input placeholder="Endpoint URL (optional)" value={endpointUrl} onChange={e => setEndpointUrl(e.target.value)} />
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            ...
            <th style={{ textAlign: 'left', borderBottom: '1px solid #ddd', padding: 8 }}>Capabilities</th>
            ...
            <tr key={a.agent_id}>
              ...
              <td style={{ padding: 8 }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  {a.description && <small style={{ color: '#666' }}>{a.description}</small>}
                  {a.endpoint_url && (
                    <small>
                      <a href={a.endpoint_url} target="_blank" rel="noreferrer">{a.endpoint_url}</a>
                    </small>
                  )}
                </div>
              </td>
              ...
              <td style={{ padding: 8, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                {canWrite(currentUser) && (
                  <button onClick={() => onActivate(a)} disabled={updatingId === a.agent_id}>
                    {updatingId === a.agent_id ? 'Saving…' : (a.status === 'active' ? 'Deactivate' : 'Activate')}
                  </button>
                )}
                {isAdmin(currentUser) && (
                  <button onClick={() => onDelete(a.agent_id)} style={{ color: '#b00' }} disabled={deletingId === a.agent_id}>
